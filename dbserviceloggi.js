            ...
            const response = await new Promise((resolve, reject) => {
                ...
                connection.query(query, [(page-1)*limit,limit], (err, results) => {
                    ...
                    for(let i = 0; i < results.length; i++){
                        results[i]["start_price"] =  Math.round((results[i]["start_price"]*ex+2000)*1.02 + 700);
                    }
                    resolve(results);
                })
            });
        ...
        try{
            ...
            const secondResponse = await new Promise((resolve, reject) => {
                ...
                connection.query(query, [id], (err, results) => {
                    if (err) reject(new Error(err.message));
                    for(let i = 0; i < results.length; i++){
                        results[i]["price"] =  Math.round((results[i]["price"]*ex+2000)*1.02 + 700);
                    }
                    resolve(results);
                })
            });
